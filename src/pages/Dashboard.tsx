            ...
            <div className="glass rounded-2xl overflow-hidden">
              {loading ? <div className="p-12 flex justify-center"><Loader2 className="animate-spin text-primary" /></div> :
                ...
                <div className="w-full overflow-x-auto overflow-y-hidden [-webkit-overflow-scrolling:touch]">
                  <table className="text-sm min-w-[900px] w-max">
                    ...
                    <tbody>
                      {pageOrders.map((o) => (
                        <tr key={o.id} className="border-t border-border/50 hover:bg-secondary/20 cursor-pointer" onClick={() => setOrderDetail(o)}>
                          <td className="px-5 py-3 font-mono text-xs whitespace-nowrap">#{String(o.order_number ?? 0).padStart(4, "0")}</td>
                          <td className="px-5 py-3 font-medium max-w-[260px] truncate" title={o.services?.name ?? undefined}>{o.services?.name ?? "—"}</td>
                          <td className="px-5 py-3 font-mono text-xs whitespace-nowrap">{o.imei}</td>
                          <td className="px-5 py-3 whitespace-nowrap"><StatusBadge status={o.status} errorMessage={sanitizeError(o.error_message)} /></td>
                          <td className="px-5 py-3 text-right font-mono whitespace-nowrap">${Number(o.price_charged).toFixed(2)}</td>
                          <td className="px-5 py-3 text-muted-foreground text-xs whitespace-nowrap">{new Date(o.created_at).toLocaleString()}</td>
        ...
        <DialogContent className="glass max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div><span className="text-muted-foreground">Amount:</span> <span className="font-mono">${Number(orderDetail?.price_charged ?? 0).toFixed(3)}</span></div>
              <div><span className="text-muted-foreground">Quantity:</span> <span>1</span></div>
              ...
              <div><span className="text-muted-foreground">Submitted On:</span> <span>{orderDetail ? new Date(orderDetail.created_at).toLocaleString() : "—"}</span></div>
              <div><span className="text-muted-foreground">Replied On:</span> <span>{orderDetail && (orderDetail.status === "completed" || orderDetail.status === "failed" || orderDetail.status === "rejected") ? new Date(orderDetail.updated_at).toLocaleString() : "—"}</span></div>
              <div className="col-span-2"><span className="text-muted-foreground">Replied In:</span> <span>{orderDetail ? formatDuration(orderDetail.created_at, orderDetail.updated_at, orderDetail.status) : "—"}</span></div>
            </div>
            <div>
